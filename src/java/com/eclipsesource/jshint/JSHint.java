...
public class JSHint {
  ...
  private static double numFiles = 0.0 ;
  private static double failedFiles = 0.0 ;
  ...
  private void load( Reader reader ) throws IOException {
    ...
    try {
      ...
      System.out.println("HERE scope: " + scope);
      context.evaluateReader( scope, reader, "jshint library", 1, null );
      System.out.println("HERE2 scope: " + scope);
      jshint = findJSHintFunction( scope );
      System.out.println("HERE3 scope: " + scope);
  ...
  private void handleProblems( ProblemHandler handler, Text text ) {
    ...
    System.out.println(jshint.get( "maxerr", jshint )) ;
    */
    Object maxErrNum = jshint.get( "maxerr", jshint ) ;
    Integer maxErr = 0 ;
    if (maxErrNum instanceof Double) {
      maxErr = ((Double)maxErrNum).intValue() ;
    } else {
      maxErr = (Integer)maxErrNum ;
    }
    //Function stateFunc = (Function)stateObject ;
    //NativeArray valOptions = (NativeArray)jshint.get( "options", stateFunc );
    //System.out.println("HERE3 scope: " + valOptions);
  ...
  private static BufferedReader getJsHintReader() throws UnsupportedEncodingException {
    ...
    String name = "jshint-" + DEFAULT_JSHINT_VERSION + ".js";
